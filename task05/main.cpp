...
void wdw_spring_3d(float& w,
  ...
  // write a few lines of code below to compute the gradient of elastic energy
  // of this spring with respect to the positions of the two end points.

  auto& p1 = node2xyz[0];
  auto& p2 = node2xyz[1];
  auto G1 = p1 - p2;
  auto G2 = -G1;
  dw[0] = 0.5 * stiffness * (2 * G1 - 2 * length_ini * G1 / G1.norm());
  dw[1] = 0.5 * stiffness * (2 * G2 - 2 * length_ini * G2 / G2.norm());
}
...
float gradient_descent_energy_minimization(
  ...
  Eigen::MatrixX3f gradW =
      Eigen::MatrixX3f::Zero(num_vtx, 3);  // gradient of the energy
  for (int i_line = 0; i_line < line2vtx.rows();
    ...
    const float length_ini = (vtx2xyz_ini.row(i_vtx0) - vtx2xyz_ini.row(i_vtx1))
                                 .norm();  // initial length
    const Eigen::Vector3f node2xyz[2] = {
        vtx2xyz.row(i_vtx0), vtx2xyz.row(i_vtx1)};  // coordinates of end points
    float w;                                        // energy of one spring
    Eigen::Vector3f dw[2] = {
        Eigen::Vector3f::Zero(),
        Eigen::Vector3f::Zero()};  // gradient of the energy of one spring
    wdw_spring_3d(  // compute energy and its gradient of a spring
        w, dw, node2xyz, length_ini, stiffness);
    W += w;
    // merge gradient
  ...
  while (!::glfwWindowShouldClose(window)) {
    ...
    for (int itr = 0; itr < 40; ++itr) {
      float W = gradient_descent_energy_minimization(
          vtx2xyz, vtx2xyz_ini, line2vtx, 60.f, 1.f, {0., -0.1, 0}, vtx2isfree,
          learning_rate);
      if (itr == 0) {
