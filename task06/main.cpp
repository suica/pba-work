/**
 * @file task06: dynamic mass-spring system using variational Euler time
 * integration
 */
...
/**
 * compute the elastic potential energy, its gradient and its hessian of a 3D
 * spring.
...
void WdWddW_Spring3(double& w,
  ...
  const double length =
      (node2xyz[0] - node2xyz[1]).norm();  // distance between p0 and p1
  const double C = length - length_ini;    // the length differences.
  w = 0.5f * stiffness * C *
      C;  // Hooke's law. energy is square of length difference W=1/2*k*C*C
  ...
  // ddw[i_node][j_node] stands for derivative of dw[i_node] w.r.t the
  // end-point's position node2xyz[j_node] the current hessian computed by the
  // code below is not very accurate, so the simulation is unstable.

  // from https://zhuanlan.zhihu.com/p/444105016
  const Eigen::Matrix3d n = stiffness * u01 * u01.transpose() +
                            stiffness * (1 - length_ini / u01.norm()) *
                                (Eigen::Matrix3d::Identity() -
                                 u01 * u01.transpose() / u01.squaredNorm());
  ddw[0][0] = n;
  ddw[1][1] = n;
  ddw[0][1] = -n;
  ddw[1][0] = -n;
...
float step_time_mass_spring_system_with_variational_integration(
  ...
  Eigen::MatrixX3d gradW =
      Eigen::MatrixX3d::Zero(num_vtx, 3);  // gradient of the energy
  sparse.setZero();
  // step position using velocity
  vtx2xyz += vtx2velocity * dt;
  for (int i_line = 0; i_line < line2vtx.rows();
       ++i_line) {                           // loop over springs
    const int i_vtx0 = line2vtx(i_line, 0);  // index of vertex 0
    const int i_vtx1 = line2vtx(i_line, 1);  // index of vertex 1
    const double length_ini =
        (vtx2xyz_ini.row(i_vtx0) - vtx2xyz_ini.row(i_vtx1))
            .norm();  // initial length
    const Eigen::Vector3d node2xyz[2] = {
        // coordinates of end points
        vtx2xyz.row(i_vtx0).cast<double>(), vtx2xyz.row(i_vtx1).cast<double>()};
    ...
    WdWddW_Spring3(w, dw, ddw, node2xyz, length_ini, stiffness);
    // merge energy
    W += w;
    // merge gradient
  ...
  for (unsigned int i_vtx = 0; i_vtx < num_vtx; ++i_vtx) {
    sparse.coeff(i_vtx, i_vtx) +=
        Eigen::Matrix3d::Identity() * (mass_point / (dt * dt));
  }
...
int main() {
  ...
  const auto [tri2vtx, vtx2xyz_ini] =
      pba::generate_mesh_annulus3(0.3, 0.8, 32, num_theta);
  const auto line2vtx =
      pba::lines_of_mesh(tri2vtx, static_cast<int>(vtx2xyz_ini.rows()));
  ...
  GLFWwindow* window = pba::window_initialization(
      "task06: dynamic mass-spring system using variational Euler time "
      "integration");
  ...
  while (!::glfwWindowShouldClose(window)) {
    if (current_time < 40.0) {
      float W = step_time_mass_spring_system_with_variational_integration(
          vtx2xyz, vtx2velocity, vtx2xyz_ini, line2vtx, 60.f, 1.f,
          {0., -0.1, 0}, vtx2isfree, dt, sparse_matrix);
      current_time += dt;
      std::cout << "time: " << current_time << "   elastic_energy: " << W
                << std::endl;
    }
    ...
    ::glMaterialfv(GL_FRONT_AND_BACK, GL_DIFFUSE,
                   std::array<float, 4>{0.0, 1.0, 1.0, 1.0}.data());
    pba::draw_triangle_3d_mesh(tri2vtx, vtx2xyz);
